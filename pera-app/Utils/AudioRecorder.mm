...
@synthesize audioBufferCallback = _audioBufferCallback;
...
-(id) init {
    ...
    _audioBufferCallback = nil;
    
    return self;
}

-(void)setDeviceID:(AudioObjectID)newDeviceID {
    if (_deviceID == newDeviceID) return;

    // only tear down if we really have an active IOProcID
    if (_IOProcID != nullptr && _deviceID != kAudioObjectUnknown) {
        [self stopIO];
    }
...
static OSStatus ioproc(AudioObjectID,
    ...
    // Call the audio buffer callback if it exists
    if (recorder.audioBufferCallback != nil && inInputData != nullptr) {
        // Create a timestamp for the callback
        AudioTimeStamp timestamp;
        memset(&timestamp, 0, sizeof(AudioTimeStamp));
        timestamp.mFlags = kAudioTimeStampSampleTimeValid;
        timestamp.mSampleTime = 0;
        
        // Call the callback on a background queue to avoid blocking the audio thread
        dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
            recorder.audioBufferCallback(inInputData, numberFramesToRecord, &timestamp);
        });
    }
